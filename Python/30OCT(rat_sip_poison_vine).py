...
def min_rats_to_identify_poisoned_bottle(num_bottles):
    import math
    # Each rat can have two states: alive or dead
    # With r rats, we can distinguish 2^r different outcomes
    # We need to find the smallest r such that 2^r >= num_bottles
    r = math.ceil(math.log2(num_bottles))
    return r
num_bottles = 1000
rats_needed = min_rats_to_identify_poisoned_bottle(num_bottles)
print(f"Minimum number of rats needed to identify the poisoned bottle among {num_bottles} bottles is: {rats_needed}")
